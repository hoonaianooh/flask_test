'''
    파이썬 <-> 데이터베이스
    파이썬으로 데이터베이스를 엑세스 하여, 쿼리를 전송, 수행결과를 받아오는 방시기
        - sql 수행
            - basic에서 수행
            - pymysql 패키지 사용
    ...
    6. 회원가입 -> insert
        - insert into [<데이터베이스명>.]<테이블명> () values ();
            [(컬럼명, 컬럼명, 컬럼명, ....)]
          values
            (값, 값, 값,...);
        - `키워드를 테이블명, 컬러명 등등 사용하면 빽틱(``보험기능)을 이용해 처리 가능`
        - '값'
        - now() : mysql의 내장함수로, 현재 시간을 리턴한다
        -
            INSERT INTO 
                `ml_db`.`users` 
...
try:
    ...
    connection = my.connect(host       ='localhost', # 127.0.0.1, 서버주소
                            # port       =3306,        # 포트
                            user       ='root',      # 사용자 계정, root계정 외의 계정 사용 권장
