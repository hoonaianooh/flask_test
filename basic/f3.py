'''
    ...
    - 방법 : method : GET, POST, PUT, ....
            -> http(or https) 프로토콜(통신규약, 서로 데이터로 주고 받는 약속)에서 정의
            ...
            post 방식은 헤더(바디의 크기가 세팅되어 있다) 전송 -> 바디 전송 -> 대량 전송 가능, 구조를 
                모르기 때문에 보안에 상대적으로 우수, 암호화 가능 -> 보안 양호(상대적)
    - 방식
        - form  전송   : <form> 태그 사용, 화면이 깜빡(순식간 진행, x로 주소창 앞에 아이콘 변경)
...
# http://127.0.0.1:5000/news3/hello -> 404 ok예상
